...
document.querySelector('.cancel-refusal-button').onclick = toggleRefusalContainer;
...
function insertIntoTable(data) {
  ...
  data.forEach((designation, index) => {
    ...
    document.getElementById(`refusal-button-${index}`).addEventListener('click', toggleRefusalContainer);
...
async function approveDesignation() {
  ...
  const result = await fetch(
    `${homeDomain}/v1/designation/approve`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
